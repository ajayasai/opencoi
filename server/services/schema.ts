...
/** Immutable checksum material for the historical-requirement table rebuild. */
export const REQUIREMENT_HISTORY_MIGRATION_SQL = `
...
/** Immutable checksum material for API-owned additive schema v1. */
export const API_ADDITIVE_SCHEMA_V1_SQL = `
...
const migrateCoverageRequirementsForHistory = (database: OpenCoiDatabase): void => {
  ...
  try {
    ...
    database.exec(REQUIREMENT_HISTORY_MIGRATION_SQL);
...
export const ensureApiSchema = (database: OpenCoiDatabase): void => {
  ...
  database.exec(API_ADDITIVE_SCHEMA_V1_SQL);
};
